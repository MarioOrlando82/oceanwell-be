<x-app-layout>
    ...
    <div class="flex min-h-[80dvh] w-full">
        <div class="m-auto h-[75dvh] w-[95vw] rounded-lg bg-[#eae6e6] p-5">
            <div class="flex h-[5%] items-center">
                <h1 class="mx-auto text-center text-3xl font-bold text-[#01283C]">Update article</h1>
            </div>
                            ...
                            <select class="mt-1 h-full rounded-md border-0 bg-[#d9d9d9] p-3" name="article_category_id"
                                ...
                                @foreach ($articleCategories as $articleCategory)
                                    <option value="{{ $articleCategory->id }}"
                                        @if ((old('article_category_id') ?? $article->article_category->id) == $articleCategory->id) selected @endif>
    ...
    @if (session('error'))
        <script>
            alertify.error('Article not updated');
        </script>
    @endif
</x-app-layout>
