    ...
    <article class="w-full">
        <header class="w-full px-14 py-8">
            ...
            <h2 class="py-2 font-semibold">By <span class="text-indigo-400">{{ $content->author_name }}</span> -
                {{ date_format($content->created_at, 'Y/m/d') }} - {{ $content->article_category->name }}
            </h2>

            <img src="{{ asset("Aset/Article/$content->image") }}" alt="banner" class="h-[30dvh] w-full object-cover">
        </header>
